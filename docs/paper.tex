...
\urlstyle{same}
\hypersetup{
  pdftitle={群衆雑音下のサッカー中継音声からのレフェリー笛検出},
  pdflang={ja},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
...
\title{群衆雑音下のサッカー中継音声からのレフェリー笛検出}
\author{}
\date{2026-06-17}
...
\textbf{― エネルギーベース手法の限界と音色ベース分類による打破 ―}

\textbf{要旨}

サッカー試合の中継音声から、レフェリーが吹く笛の時刻を自動検出する課題に取り組んだ。
笛は約2
...
ある。サッカーにおいてレフェリーの笛は反則・得点・前後半の開始終了など多くの局面を
...
\textbf{F1は56\%→72\%(+16ポイント)}に向上した。回帰係数の大きい特徴は、立ち上がりの鋭さ
(onset, 最大の正係数)、低域比(群衆の少なさ,
負係数)、倍音比、持続であり、「笛らしい音色」を
学習していることを示す。再現率上限は候補集合に依存するため90\%のままで、分類器は
...
\textbf{F1
74\%→91\%}。音色特徴が試合(ハーフ)をまたいで有効であることを示す。
